...
class Katalog extends MY_Controller
{
  ...
  function gallery()
  {
    ...
    $this->render('admin/v_katalog_gallery', $data);
  }
  ...
  {
    $data = [
      'data' => $this->m_crud->read('tbl_stok_barang')
    ];
    $this->render('admin/v_katalog_inventori', $data);
  }

  public function simpan_inventori()
  {
    $data = [
    ...
    if (!$this->input->post('row_id_brg')) {
      $this->m_crud->insert('tbl_stok_barang', $data);
      ...
      redirect('admin/katalog/inventori');
    }
    $this->m_crud->update('tbl_stok_barang', $data, 'row_id_brg', $this->input->post('row_id_brg'));
    $this->session->set_flashdata('msg', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><b>' . $data['nm_brg'] . '</b> Berhasil ditambah/diupdate.</div>');
    redirect('admin/katalog/inventori');
  }

  public function hapus_inventori($row_id_brg)
  {
    $this->m_crud->delete('tbl_stok_barang', 'row_id_brg', $row_id_brg);
